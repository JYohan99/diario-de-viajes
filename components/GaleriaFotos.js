  ...
  return (
    ...
    <View style={s.contenedor}>
      ...
      <ModalGPS
        visible={modalGPS}
        ubicacionTemp={ubicacionTemp}
        onSeleccionarUbicacion={(e) => setUbicacionTemp(e.nativeEvent.coordinate)}
        onConfirmar={() => {
          if (ubicacionTemp) {
            ...
            <View style={s.filaAcciones}>
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  ...
                  if (seleccionadasGPS.length === 0 || !ubicacionParaAplicar) return;
                  ...
                  for (const id of idsAActualizar) {
                    ...
                    if (act) actuales = actuales.map((x) => (x.id === id ? act : x));
                  }
                  setFotos(actuales);
                  Alert.alert('✅ Listo', `Ubicación aplicada a ${idsAActualizar.length} foto(s).`);
