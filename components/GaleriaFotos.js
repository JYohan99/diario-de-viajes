...
function agruparPorCarpeta(fotos) {
  const grupos = {};
  fotos.forEach((foto) => {
...
export default function GaleriaFotos() {
  ...
  function abrirEditar(foto) {
    ...
    setUbicacionTemp(
      foto.latitud ? { latitude: foto.latitud, longitude: foto.longitud } : null
    );
  ...
  return (
    <View style={s.contenedor}>
                    ...
                    <Text style={s.valorDetalle}>
                      {fotoAmpliada.tamaño > 0
                        ? fotoAmpliada.tamaño > 1024 * 1024
                          ? `${(fotoAmpliada.tamaño / (1024 * 1024)).toFixed(
                              1
                            )} MB`
      ...
      {/* ── Modal mapa GPS ── */}
      <Modal visible={modalGPS} transparent={false} animationType="slide">
        <View style={{ flex: 1, backgroundColor: colores.fondoOscuro }}>
          <View style={s.encabezadoGPS}>
            <TouchableOpacity onPress={() => setModalGPS(false)}>
          ...
          <MapView
            ...
            onPress={(e) => setUbicacionTemp(e.nativeEvent.coordinate)}>
            {ubicacionTemp && (
          ...
          <View style={[s.tarjeta, { maxHeight: '85%' }]}>
            ...
            <View style={s.filaAcciones}>
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  let actuales = [...fotos];
                  for (const id of seleccionadasGPS) {
                    const act = await actualizarFoto(id, {
                      latitud: ubicacionParaAplicar.latitud,
                      longitud: ubicacionParaAplicar.longitud,
                    });
                    if (act)
                  ...
                  Alert.alert(
                    '✅ Listo',
                    `Ubicación aplicada a ${seleccionadasGPS.length} foto(s).`
                  );
                }}>
